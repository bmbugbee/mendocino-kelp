...
noyo_balance <- noyo_benthic_data_cleaned %>%
  ...
  group_by(Date, month, ARKEV_type, ARKEV_unit, species_code) %>%  # 🔴 KEY STEP
  summarise(
...
#PERMANOVA
#adonis2(comm_unit ~ ARKEV_type, data = meta_unit, method = "bray")
#

#plot NMDS
site_scores <- as.data.frame(vegan::scores(nmds_noyo_unit, display = "sites")) %>%
  bind_cols(meta_unit)
...
all_balance <- all_sites_benthic_data_cleaned %>%
  ...
  group_by(site,Date, month, ARKEV_type, ARKEV_unit, species_code) %>%  # 🔴 KEY STEP
  summarise(
    density = sum(density, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
#plot NMDS
site_scores <- as.data.frame(vegan::scores(nmds_all_unit, display = "sites")) %>%
  bind_cols(meta_unit)
...
ggplot() +
  ...
  scale_color_manual(values = c("Noyo" = "#edf8b1", "Big_River" = "#2c7fb8")) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    legend.position = "right",
...
scores_sites <- as.data.frame(vegan::scores(nmds_all_unit, display = "sites"))
scores_sites <- cbind(scores_sites, meta_unit)

scores_species <- as.data.frame(vegan::scores(species_fit, display = "vectors"))
...
```
#Urchin Density over Time
```{r}
urchin_balance <- all_balance %>%
  filter(ARKEV_type != "ARKEV_control") %>%
  select(c(site, month, ARKEV_unit, MESFRA, `STRPUR_<2cm`, `STRPUR_>2cm`))

plot_df <- urchin_balance %>%
  pivot_longer(
    cols = c(MESFRA, `STRPUR_<2cm`, `STRPUR_>2cm`),
    names_to = "metric",
    values_to = "density"
  ) %>%
  mutate(
    species = case_when(
      metric == "MESFRA" ~ "Red urchins",
      TRUE               ~ "Purple urchins"
    ),
    size_class = case_when(
      metric == "MESFRA"        ~ "all",
      metric == "STRPUR_<2cm"   ~ "<2cm",
      metric == "STRPUR_>2cm"   ~ ">2cm"
    )
  ) %>%
  group_by(site, month, species, size_class) %>%
  summarise(mean_density = mean(density, na.rm = TRUE), .groups = "drop")

purple_df <- plot_df %>% filter(species == "Purple urchins")
red_df    <- plot_df %>% filter(species == "Red urchins")

ggplot() +
  # Purple: color = site, linetype = size class
  geom_line(data = purple_df,
            aes(x = month, y = mean_density, color = site, linetype = size_class),
            linewidth = 1) +
  geom_point(data = purple_df,
             aes(x = month, y = mean_density, color = site, linetype = size_class),
             size = 2) +

  # Red: color = site only (no linetype mapping)
  geom_line(data = red_df,
            aes(x = month, y = mean_density, color = site),
            linewidth = 1) +
  geom_point(data = red_df,
             aes(x = month, y = mean_density, color = site),
             size = 2) +

  facet_grid(species ~ ., scales = "free_y") +
  scale_x_continuous(breaks = sort(unique(plot_df$month))) +
  labs(x = "Month", y = "Mean density (indv/m^2)", color = "Site", linetype = "Purple size class") +
  theme_bw()
```
...
fish <- all_sites_cleaned %>%
filter(data_type %in% c("fish_on", "fish_off")) %>%
  filter(ARKEV_type != "ARKEV_control") %>%
  select(c(year, month,day, site, ARKEV_type, ARKEV_unit, data_type, species_code, count)) %>%
  mutate(date = make_date(year, month, day))
...
sum_fish <- fish %>%
  group_by(month, date, site, ARKEV_type, ARKEV_unit, data_type) %>%
  summarise(fish_density = sum(count[species_code != "NO_FISH"], na.rm = TRUE),
            species_richness = n_distinct(species_code[species_code != "NO_FISH" & count > 0]),
            .groups = "drop")
View(sum_fish)
...
###per month
```{r}
fish_monthly <- sum_fish %>%
  group_by(data_type, month, site) %>%
summarise(
...
ggplot(fish_monthly, aes(x = month, y = mean_richness, color = site, linetype = data_type)) +
  ...
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = mean_richness - se_richness, ymax = mean_richness + se_richness), width = 0.1) +
  scale_x_continuous(breaks = sort(unique(fish_monthly$month))) +
   scale_color_manual(values = trt_loc_colors) +
  labs(title = "Mean Species Richness of Fish by Month", x = "Month", y = "Mean species richness", color = "Site", linetype = "Location") +
  theme_bw(base_size = 14) 
...
ggplot(fish_monthly,
  ...
  scale_x_continuous(breaks = sort(unique(fish_monthly$month))) +
  scale_linetype_manual(
    values = c("fish_off" = "dashed", "fish_on" = "solid")
  ) +
  labs(
      title = "Average Species Richness of Fish per Month",
    x = "Month",
...
mean_fish <- mean_fish %>%
  mutate(data_type = factor(data_type, levels = c("fish_on", "fish_off")))
...
ggplot(mean_fish,
       aes(x = site, y = mean_richness, fill = data_type)) +
  geom_col(position = pd, color = "black", width = 0.7) +
  geom_errorbar(
    aes(ymin = mean_richness - se_richness,
        ymax = mean_richness + se_richness),
    position = pd, width = 0.2, linewidth = 0.7
  ) +
  scale_fill_manual(values = loc_colors) +
  theme_bw(base_size = 14) +
  labs(
    title = "Mean Species Richness of Fish by Site",
    x = "Site",
    y = "Mean Species Richness",
    fill = "Location"
  )
...
##Rockfish Density
```{r}
rockfish_codes <- c("KGBC", "OYT", "SMEL", "YOY", "SMYS", "SCAU", "SPIN", "SATR", "SCAR")

rockfish <- fish %>%
  group_by(month, date, site, ARKEV_type, ARKEV_unit, data_type) %>%
  summarise(
    fish_density = sum(count[species_code %in% rockfish_codes], na.rm = TRUE),
    species_richness = n_distinct(
      species_code[species_code %in% rockfish_codes & count > 0]
    ), n_units      = n(),
    .groups = "drop"
  )

# sanity check: do you actually have zero units included?
rockfish %>% summarise(
  n_units = n(),
  n_zero_density = sum(fish_density == 0)
)


rockfish_density_monthly <- rockfish %>%
  group_by(data_type, month, site) %>%
  summarise(
...
ggplot(rockfish_density_monthly, aes(x = month, y = mean_density, color = site, linetype = data_type)) +
  scale_linetype_manual(
  values = c(
    "fish_on"  = "solid",
    "fish_off" = "dashed")) + 
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = mean_density - se_density, ymax = mean_density + se_density), width = 0.1) +
  scale_x_continuous(breaks = sort(unique(rockfish_density_monthly$month))) +
   scale_color_manual(values = trt_loc_colors) +
  labs(title = "Average Abundance of Rockfish by Month", x = "Month", y = "Average Abundance", color = "Site", linetype = "Location") +
  theme_bw(base_size = 14) 
```
...
rockfish_density_monthly <- rockfish_density_monthly %>%
  mutate(data_type = factor(data_type, levels = c("fish_on", "fish_off")))
...
ggplot(rockfish_density_monthly,
       aes(x = site, y = mean_density, fill = data_type)) +
  geom_col(position = pd, color = "black", width = 0.7) +
  geom_errorbar(
  ...
  theme_bw(base_size = 14) +
  labs(
    title = "Average Abundance of Rockfish by Site",
    x = "Site",
    y = "Average Abundance",
    fill = "Location"
  )
