...
View(benthic_data_cleaned)
```

#community matrix
```{r}
library(vegan)
benthic_comm <- benthic_data_cleaned %>%
  # 1. (optional but common) aggregate within samples × species
  group_by(Date, month, ARKEV_type, ARKEV_unit, species_code) %>%
  summarise(density = sum(density, na.rm = TRUE), .groups = "drop") %>%
  
  # 2. pivot to wide community matrix
  pivot_wider(
...
benthic_comm_species <- benthic_comm %>%
  dplyr::select(-Date, -month, -ARKEV_type, -ARKEV_unit)


comm_cols <- c(
  "STRPUR_>2cm", "LAMR", "TEGULA",
  "MESFRA", "ALAR", "ANEMONE", "CANSPP",
  "CORCAL", "COSCOS",
  "DERIMB", "DESSPP", "DICTYOTALES",
  "HALRUF", "LAMR", "LAMSET", "NERA",
  "NERJ", "PATMIN", "PISBRE", "PISGIG", "PISOCH", "STRPUR_<2cm", "STUMP"
)

comm_matrix <- benthic_comm[,comm_cols]
