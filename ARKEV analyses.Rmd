...
#Kelp biomass data
```{r}
biomass = read_sheet("https://docs.google.com/spreadsheets/d/17-pjiXi3e52YEpnj5WyhM5IK9DjtKiVqnL2YJaOYYhU/edit?gid=0#gid=0")
...
all_benthic_data <- both_years_MASTER %>%
  ...
  mutate(
    density = case_when(
      year == 2024 ~ count,          # already per m²
      year == 2025 ~ count / pi,     # convert 1 m radius counts to per m²
      TRUE ~ NA_real_
    )
  )
...
all_benthic_comm_balance <- all_benthic_data_cleaned %>%
  ...
  group_by(year,Date, month, ARKEV_type, ARKEV_unit, species_code) %>%  # 🔴 KEY STEP
  summarise(
    density = sum(density, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
geom_text_repel(
  ...
  theme_classic() +
  labs(
    title = "NMDS – Yearly Comparison (2024 vs 2025)",
    color = "Year"
  ) +
  scale_color_manual(values = c("2024" = "darkblue", "2025" = "red3"))
...
benthic_comm_time <- all_benthic_comm_balance %>%
  group_by(year, month, ARKEV_type, ARKEV_unit) %>%  # unit × month × year
  summarise(
    across(MESFRA:PARCAL, mean, na.rm = TRUE),
    .groups = "drop"
  )
...
## 1. Monthly centroids (one point per month × year)
month_centroids <- site_scores_time %>%
  group_by(year, month) %>%
  summarise(
    NMDS1 = mean(NMDS1),
...
## 1. Make sure you’re using the same community matrix used for nmds_time
##    Here I assume it’s called `comm_time_ne`.
...
ggplot(emm_df,
  ...
  labs(
    ...
    x = "Treatment",
    color = "Location",
    title = "Treatment × Location Interaction (Negative Binomial GLMM)"
  )  +
  theme_bw(base_size = 14)
```
