...
```{r}
inv_comm <- benthic_data %>%
  filter(data_type == "inverts") %>%
  mutate(count = replace_na(count, 0)) %>%   # avoid NA counts
  pivot_wider(
...
benthic_comm <- benthic_block %>%
  group_by(month, ARKEV_type, block, species_code) %>%
...
permanova <- adonis2(comm_trans ~ ARKEV_type + month, data = meta, method = "bray")
permanova
...
```
##community matrix by treatment
```{r}
benthic_comm_treat <- benthic_data_cleaned %>%
  mutate(
    ARKEV_type = case_when(
      ARKEV_unit %in% c("2A", "2C", "2E", "2D", "2B",
                        "3A", "3C", "3E", "3B", "3D",
  ...
  dplyr::select(-NO_INVERTS, -NO_KELP) 
...
ggplot() +
  ...
  scale_color_manual(values = c("ARKEV" = "blue", "ARKEV_control" = "red", "ARKEV_drift" = "green3")) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    legend.position = "right",
...
```
##community matrix by quadrant 
```{r}
benthic_comm_quad <- benthic_data_cleaned %>%
...
meta <- benthic_comm_quad %>%
  dplyr::select(ARKEV_type, ARKEV_unit)

comm <- benthic_comm_quad %>%
  dplyr::select(-ARKEV_type, -ARKEV_unit)
...
ggplot() +
  ...
  scale_color_manual(values = c("ARKEV_NW" = "blue", "ARKEV_SW" = "red", "ARKEV_NE" = "green3", "ARKEV_SE" = "purple3")) +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    legend.position = "right",
...
```
##community matrix temporal 
```{r}
benthic_comm_month <- benthic_data_cleaned %>% 
  mutate(
    ARKEV_type = case_when(
      ARKEV_unit %in% c("2A", "2C", "2E", "2D", "2B",
                        "3A", "3C", "3E", "3B", "3D",
