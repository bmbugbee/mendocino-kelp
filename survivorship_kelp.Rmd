...
```{r}
noyo_kelp <- read_sheet("https://docs.google.com/spreadsheets/d/15VQ4LNt9-319UO0C1MFzv_K2XmGeYXjLD0d7JmLxxEU/edit?gid=0#gid=0") |>
  mutate(site = "Noyo") |>
...
```{r cars}
br_kelp <- read_sheet("https://docs.google.com/spreadsheets/d/1srKfto1CpjAfJav2ofoTxPsC_3MmxAIfdA7utfCT5p0/edit?gid=0#gid=0") %>%
...
br_clean <- br_kelp |>
  ...
  group_by(date, site) |>
  mutate(total = sum(kelp_on_ARKEV, na.rm = TRUE))
...
```{r}
full_kelp = read_sheet("https://docs.google.com/spreadsheets/d/1neggPBLv4m3AwTWzQxNBIz0ALO0S8s-IIYeUeH502RU/edit?gid=0#gid=0") %>%
   filter(!is.na(kelp_on_ARKEV)) |>
  mutate(stipe_length = as.numeric(stipe_length),
         kelp_on_ARKEV = as.numeric(kelp_on_ARKEV),
         reproductive_kelp_on_ARKEV = as.numeric(reproductive_kelp_on_ARKEV),
